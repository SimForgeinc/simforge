...
- **`SimForgeVector`** — synchronous vector of N sessions on ONE server
...
`packages/training-env/src/env-server.ts`; `backend="ts"` names the TypeScript
...
vec = SimForgeVector("episodes.json", num_envs=8)
...
The client spawns the server automatically (`simforge-env-server` on
