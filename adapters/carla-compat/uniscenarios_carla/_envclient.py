...
Mirrors the wire protocol of ``adapters/uniscenarios-gym``: 4-byte
...
_MAIN_SERVER_DIST = Path("/home/path/UniScenarios/packages/rl-env/dist/env-server.js")
...
def resolve_server_command(server_command=None) -> tuple[str, ...]:
    ...
    raise RuntimeError(
        "no uniscenarios-env-server found: install @uniscenarios/rl-env "
        "(pnpm --filter @uniscenarios/rl-env build), set UNISCENARIO_ENV_SERVER, "
