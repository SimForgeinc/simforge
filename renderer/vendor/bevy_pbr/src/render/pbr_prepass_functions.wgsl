...
// Raw texture coordinates of the prepass vertex output; the prepass never
// applies parallax, so the displacement terms are zero.
fn prepass_slot_uv_inputs(in: VertexOutput) -> pbr_types::SlotUvInputs {
    var inputs: pbr_types::SlotUvInputs;
#ifdef VERTEX_UVS_A
    inputs.raw[0] = in.uv;
#else
    inputs.raw[0] = vec2<f32>(0.0);
#endif
#ifdef VERTEX_UVS_B
    inputs.raw[1] = in.uv_b;
#else
    inputs.raw[1] = inputs.raw[0];
#endif
#ifdef VERTEX_UVS_C
    inputs.raw[2] = in.uv_c;
#else
    inputs.raw[2] = inputs.raw[0];
#endif
#ifdef VERTEX_UVS_D
    inputs.raw[3] = in.uv_d;
#else
    inputs.raw[3] = inputs.raw[0];
#endif
    inputs.parallax_delta[0] = vec2<f32>(0.0);
    inputs.parallax_delta[1] = vec2<f32>(0.0);
    return inputs;
}
...
#endif  // BINDLESS
    let uv = pbr_types::standard_material_slot_uv(slot_material, pbr_types::STANDARD_MATERIAL_SLOT_BASE_COLOR, prepass_slot_uv_inputs(in));
