...
use bevy_math::{Affine2, Affine3, Mat2, Mat3, UVec4, Vec2, Vec3, Vec4};
...
pub struct StandardMaterial {
    ...
    ///
    /// Every texture slot samples with this transform unless it carries its own
    /// `*_uv_transform` (glTF `KHR_texture_transform` is per slot).
    pub uv_transform: Affine2,

    /// Per-slot UV transform for `base_color_texture`; `None` uses [`Self::uv_transform`].
    pub base_color_uv_transform: Option<Affine2>,

    /// Per-slot UV transform for `emissive_texture`; `None` uses [`Self::uv_transform`].
    pub emissive_uv_transform: Option<Affine2>,

    /// Per-slot UV transform for `metallic_roughness_texture`; `None` uses [`Self::uv_transform`].
    pub metallic_roughness_uv_transform: Option<Affine2>,

    /// Per-slot UV transform for `normal_map_texture`; `None` uses [`Self::uv_transform`].
    pub normal_map_uv_transform: Option<Affine2>,

    /// Per-slot UV transform for `occlusion_texture`; `None` uses [`Self::uv_transform`].
    pub occlusion_uv_transform: Option<Affine2>,

    /// Per-slot UV transform for `specular_transmission_texture`; `None` uses [`Self::uv_transform`].
    #[cfg(feature = "pbr_transmission_textures")]
    pub specular_transmission_uv_transform: Option<Affine2>,

    /// Per-slot UV transform for `thickness_texture`; `None` uses [`Self::uv_transform`].
    #[cfg(feature = "pbr_transmission_textures")]
    pub thickness_uv_transform: Option<Affine2>,

    /// Per-slot UV transform for `diffuse_transmission_texture`; `None` uses [`Self::uv_transform`].
    #[cfg(feature = "pbr_transmission_textures")]
    pub diffuse_transmission_uv_transform: Option<Affine2>,

    /// Per-slot UV transform for `specular_texture`; `None` uses [`Self::uv_transform`].
    #[cfg(feature = "pbr_specular_textures")]
    pub specular_uv_transform: Option<Affine2>,

    /// Per-slot UV transform for `specular_tint_texture`; `None` uses [`Self::uv_transform`].
    #[cfg(feature = "pbr_specular_textures")]
    pub specular_tint_uv_transform: Option<Affine2>,

    /// Per-slot UV transform for `clearcoat_texture`; `None` uses [`Self::uv_transform`].
    #[cfg(feature = "pbr_multi_layer_material_textures")]
    pub clearcoat_uv_transform: Option<Affine2>,

    /// Per-slot UV transform for `clearcoat_roughness_texture`; `None` uses [`Self::uv_transform`].
    #[cfg(feature = "pbr_multi_layer_material_textures")]
    pub clearcoat_roughness_uv_transform: Option<Affine2>,

    /// Per-slot UV transform for `clearcoat_normal_texture`; `None` uses [`Self::uv_transform`].
    #[cfg(feature = "pbr_multi_layer_material_textures")]
    pub clearcoat_normal_uv_transform: Option<Affine2>,

    /// Per-slot UV transform for `anisotropy_texture`; `None` uses [`Self::uv_transform`].
    #[cfg(feature = "pbr_anisotropy_texture")]
...
impl Default for StandardMaterial {
    fn default() -> Self {
        StandardMaterial {
            ...
            #[cfg(feature = "pbr_transmission_textures")]
            diffuse_transmission_uv_transform: None,
            #[cfg(feature = "pbr_specular_textures")]
...
pub struct StandardMaterialUniform {
    ...
    /// Per-slot UV transforms, indexed by `STANDARD_MATERIAL_SLOT_*` (see `pbr_types.wgsl`).
    pub slot_uv_transforms: [Mat3; STANDARD_MATERIAL_SLOT_COUNT],
    /// Per-slot UV channel (`UvChannel::index`), four bits per slot, eight slots per word.
    pub slot_uv_channels: UVec4,
}

/// Number of texture slots with their own UV channel and transform.
pub const STANDARD_MATERIAL_SLOT_COUNT: usize = 14;

/// Packs one `UvChannel` per slot into four bits each, eight slots per word.
fn pack_slot_channels(channels: [UvChannel; STANDARD_MATERIAL_SLOT_COUNT]) -> UVec4 {
    let mut words = [0u32; 4];
    for (slot, channel) in channels.into_iter().enumerate() {
        words[slot / 8] |= channel.index() << ((slot % 8) * 4);
    }
    UVec4::from_array(words)
}
...
impl AsBindGroupShaderType<StandardMaterialUniform> for StandardMaterial {
    fn as_bind_group_shader_type(
        ...
        StandardMaterialUniform {
            ...
            slot_uv_transforms: [
                self.base_color_uv_transform.unwrap_or(self.uv_transform).into(),
                self.emissive_uv_transform.unwrap_or(self.uv_transform).into(),
                self.metallic_roughness_uv_transform.unwrap_or(self.uv_transform).into(),
                self.normal_map_uv_transform.unwrap_or(self.uv_transform).into(),
                self.occlusion_uv_transform.unwrap_or(self.uv_transform).into(),
                {
                    #[cfg(feature = "pbr_transmission_textures")]
                    { self.specular_transmission_uv_transform.unwrap_or(self.uv_transform).into() }
                    #[cfg(not(feature = "pbr_transmission_textures"))]
                    { self.uv_transform.into() }
                },
                {
                    #[cfg(feature = "pbr_transmission_textures")]
                    { self.thickness_uv_transform.unwrap_or(self.uv_transform).into() }
                    #[cfg(not(feature = "pbr_transmission_textures"))]
                    { self.uv_transform.into() }
                },
                {
                    #[cfg(feature = "pbr_transmission_textures")]
                    { self.diffuse_transmission_uv_transform.unwrap_or(self.uv_transform).into() }
                    #[cfg(not(feature = "pbr_transmission_textures"))]
                    { self.uv_transform.into() }
                },
                {
                    #[cfg(feature = "pbr_specular_textures")]
                    { self.specular_uv_transform.unwrap_or(self.uv_transform).into() }
                    #[cfg(not(feature = "pbr_specular_textures"))]
                    { self.uv_transform.into() }
                },
                {
                    #[cfg(feature = "pbr_specular_textures")]
                    { self.specular_tint_uv_transform.unwrap_or(self.uv_transform).into() }
                    #[cfg(not(feature = "pbr_specular_textures"))]
                    { self.uv_transform.into() }
                },
                {
                    #[cfg(feature = "pbr_multi_layer_material_textures")]
                    { self.clearcoat_uv_transform.unwrap_or(self.uv_transform).into() }
                    #[cfg(not(feature = "pbr_multi_layer_material_textures"))]
                    { self.uv_transform.into() }
                },
                {
                    #[cfg(feature = "pbr_multi_layer_material_textures")]
                    { self.clearcoat_roughness_uv_transform.unwrap_or(self.uv_transform).into() }
                    #[cfg(not(feature = "pbr_multi_layer_material_textures"))]
                    { self.uv_transform.into() }
                },
                {
                    #[cfg(feature = "pbr_multi_layer_material_textures")]
                    { self.clearcoat_normal_uv_transform.unwrap_or(self.uv_transform).into() }
                    #[cfg(not(feature = "pbr_multi_layer_material_textures"))]
                    { self.uv_transform.into() }
                },
                {
                    #[cfg(feature = "pbr_anisotropy_texture")]
                    { self.anisotropy_uv_transform.unwrap_or(self.uv_transform).into() }
                    #[cfg(not(feature = "pbr_anisotropy_texture"))]
                    { self.uv_transform.into() }
                },
            ],
            slot_uv_channels: pack_slot_channels([
                self.base_color_channel,
                self.emissive_channel,
                self.metallic_roughness_channel,
                self.normal_map_channel,
                self.occlusion_channel,
                {
                    #[cfg(feature = "pbr_transmission_textures")]
                ...
                {
                    #[cfg(feature = "pbr_specular_textures")]
                    { self.specular_channel }
                    #[cfg(not(feature = "pbr_specular_textures"))]
                    { UvChannel::Uv0 }
                },
                {
                    #[cfg(feature = "pbr_specular_textures")]
                    { self.specular_tint_channel }
                    #[cfg(not(feature = "pbr_specular_textures"))]
                    { UvChannel::Uv0 }
                },
                {
                    #[cfg(feature = "pbr_multi_layer_material_textures")]
                    { self.clearcoat_channel }
                    #[cfg(not(feature = "pbr_multi_layer_material_textures"))]
                    { UvChannel::Uv0 }
                },
                {
