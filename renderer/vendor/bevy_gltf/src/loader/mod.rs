...
pub enum GltfError {
    ...
    /// A texture references no image, neither through the core `source` field
    /// nor through the `KHR_texture_basisu` extension.
    #[error("texture {0} references no image (missing both core `source` and `KHR_texture_basisu.source`)")]
    #[from(ignore)]
    MissingTextureSource(usize),
    /// A texture declares `KHR_texture_basisu` but its `source` is missing,
    /// not an integer, or out of range.
    #[error("texture {0} has a malformed `KHR_texture_basisu.source`")]
    #[from(ignore)]
    InvalidBasisuSource(usize),
...
async fn load_image<'a, 'b>(
    ...
    let image = match texture_source(&gltf_texture, document) {
        TextureSource::Image(image) => image,
        TextureSource::Missing => {
            return Err(GltfError::MissingTextureSource(gltf_texture.index()));
        }
        TextureSource::InvalidBasisu => {
            return Err(GltfError::InvalidBasisuSource(gltf_texture.index()));
        }
    };
    match image.source() {
...
"#,
    ...
    /// A texture without core `source` must resolve its image through
    /// `extensions.KHR_texture_basisu.source`, with glTF validation enabled
    /// (`KHR_texture_basisu` listed in `extensionsRequired`).
    /// See <https://github.com/bevyengine/bevy/issues/19104>.
    #[test]
    fn khr_texture_basisu_source_resolves_without_core_source() {
        let (mut app, dir) = test_app_custom_asset_source();
...
"#,
        ...
        // The KTX2 payload is never decoded here: the URI branch defers to a
        // nested image load, which this fake loader satisfies. This keeps the
        // test independent of image-format features.
        dir.insert_asset_text(Path::new("abc.ktx2"), "not a real ktx2");

        /// A fake loader to avoid actually decoding KTX2 data.
        #[derive(TypePath)]
        ...
        let gltf_root_assets = app.world().resource::<Assets<Gltf>>();
        let gltf_material_assets = app.world().resource::<Assets<GltfMaterial>>();
        let gltf_root = gltf_root_assets.get(&handle).unwrap();
        assert_eq!(gltf_root.materials.len(), 1);
        let material = gltf_material_assets
            .get(&gltf_root.materials[0])
            .unwrap();
        let texture = material
            .base_color_texture
            .as_ref()
            .expect("base color texture resolved through KHR_texture_basisu.source");
        let path = texture.path().expect("image loaded by path").to_string();
        assert!(
            path.ends_with("abc.ktx2"),
            "texture must reference the extension's image, got {path}"
        );
    }

    /// A texture referencing no image at all must fail the load loudly
    /// instead of panicking or silently dropping the texture.
    #[test]
    fn texture_without_any_source_fails_loudly() {
        let (mut app, dir) = test_app_custom_asset_source();
...
"#,
        ...
        run_app_until(&mut app, |_| match asset_server.load_state(&handle) {
            LoadState::Failed(err) => {
                let err = err.to_string();
                assert!(
                    err.contains("references no image"),
                    "incorrect error message: {err}"
                );
                Some(())
            }
