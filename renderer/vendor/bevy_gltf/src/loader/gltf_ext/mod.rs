...
pub(crate) fn get_linear_textures(document: &Document) -> HashSet<usize> {
    ...
    for material in document.materials() {
        ...
        // `specularTexture` is a linear strength map (its alpha channel);
        // `specularColorTexture` is sRGB color and stays out of this set.
        #[cfg(feature = "pbr_specular_textures")]
        if let Some(texture_index) =
            extension_texture_index(&material, "KHR_materials_specular", "specularTexture")
        {
            linear_textures.insert(texture_index);
        }

        // Transmission and volume thickness maps are linear data.
        #[cfg(feature = "pbr_transmission_textures")]
        {
            if let Some(texture_index) = extension_texture_index(
                &material,
            ...
            if let Some(texture_index) =
                extension_texture_index(&material, "KHR_materials_volume", "thicknessTexture")
            {
                linear_textures.insert(texture_index);
            }
        }
