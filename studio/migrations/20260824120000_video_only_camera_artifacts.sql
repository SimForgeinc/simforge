-- migration-impact: non-destructive
-- migration-window: browser recordings are video-only for cameras — every render-spec/v3
-- source owns exactly one sensor_video stream (cameras always; lidar/radar when a video
-- output is requested) and sensor_archive rows exist only for lidar/radar measurement
-- data. Individual camera frame archives were removed outright.
BEGIN;
  ...
  IF EXISTS (
    WITH expected(role, actor_id, sensor_id, modality, relationship) AS (
      ...
      SELECT 'sensor_video', source->>'actorId', source->>'sensorId',
       ...
       WHERE spec_schema = 'uniscenario.render-spec/v3'
         AND (
           source->>'modality' NOT IN ('lidar', 'radar')
           OR spec ? 'video'
         )
      UNION ALL
...
VALUES ('20260824120000_video_only_camera_artifacts.sql')
