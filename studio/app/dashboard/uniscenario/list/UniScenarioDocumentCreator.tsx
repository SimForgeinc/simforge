...
import {
  ...
  type UniScenarioMapGroup,
  type UniScenarioMapOption,
} from "./document-map-groups";
import { UNISCENARIO_TAG_DRAG_MIME, UniScenarioDocumentRow } from "./UniScenarioDocumentRow";
import {
  DEFAULT_UNISCENARIO_TAG_COLOR,
  DEFAULT_UNISCENARIO_TAG_COLORS,
  uniScenarioListCache,
} from "./uniScenarioListCache";

type SharedRowHandlers = Pick<
  React.ComponentProps<typeof UniScenarioDocumentRow>,
  | "onAssignTag"
  | "onRenameDraftChange"
...
export type UniScenarioDocumentCreatorProps = SharedRowHandlers & {
  datasetId: string;
  ...
  availableMaps: ReadonlyArray<UniScenarioMapOption>;
 ...
 */
export function UniScenarioDocumentCreator({
  datasetId,
  datasetEditable,
  documents,
  totalDocumentCount,
  ...
  renderInProgress,
  ...rowHandlers
}: UniScenarioDocumentCreatorProps) {
  ...
  const [expandedMapKeys, setExpandedMapKeys] = useState<Set<string>>(
    () => new Set([...(uniScenarioListCache.expandedMapLabelsByDataset[datasetId] ?? [])].slice(0, 1)),
  );
  ...
  const toggleVariationsFor = (document: UniScenarioDocumentSummaryDto) => {
  ...
  const renderRow = (
    ...
    return (
      <div key={document.id} data-uniscenario-with-variations="">
        {row}
        <div
          className="ml-4 border-l border-primary/30"
          data-uniscenario-variations-sublist=""
        >
  ...
  return (
    <div
      ...
      data-uniscenario-advanced-mode={advancedMode ? "" : undefined}
    >
      {showTagEditorTools ? (
        <UniScenarioTagTools
          tags={tags}
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto" data-testid="uniscenario-document-list">
...
function UniScenarioTagTools({
  ...
  return (
    <aside
      ...
      data-uniscenario-tag-tools-mode={tagEditorMode ? "editor" : "filter"}
    >
      <div className="space-y-3">
        <section className="relative space-y-1.5">
          ...
          {tagEditorMode && createOpen ? (
            <form
              data-testid="uniscenario-tag-create-form"
        ...
        <section className="space-y-1.5">
          ...
          <div className="flex flex-col gap-1.5">
            {tags.length === 0 ? (
              ...
              tags.map((tag) => {
                const color = tag.color ?? DEFAULT_UNISCENARIO_TAG_COLOR;
                ...
                return (
                  <div
                    ...
                    data-uniscenario-tag-id={tag.id}
                  >
                    {editingTagId === tag.id ? (
                      <input
                        autoFocus
...
function UniScenarioTagColorPicker({
  selectedColor,
  onSelectColor,
  label,
}: {
  ...
  const activeColor = (selectedColor ?? DEFAULT_UNISCENARIO_TAG_COLOR).toLowerCase();
  return (
    <div className="flex flex-wrap gap-1" role="group" aria-label={label}>
      {DEFAULT_UNISCENARIO_TAG_COLORS.map((color) => (
        <button
          key={color}
          type="button"
          className={cn(
...
function startTagDrag(
  ...
  tag: UniScenarioTagDto,
  onBeginTagDrag: (tagId: string) => void,
) {
  onBeginTagDrag(tag.id);
  event.dataTransfer.effectAllowed = "copy";
  event.dataTransfer.setData(UNISCENARIO_TAG_DRAG_MIME, tag.id);
