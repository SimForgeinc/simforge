 ...
 *
 * The three steps are the pipeline, not a wrapper: `createUniScenarioRevision` freezes an immutable
 ...
 */
export function useUniScenarioOpenScenarioExport({
  documentId,
  onError,
  onNotice,
}: {
  ...
  const exportOpenScenario = useCallback(async () => {
    ...
    try {
      ...
      // row carries no `draftVersion`, and `createUniScenarioRevision` needs one.
      ...
      const created = await createUniScenarioRevision(document, crypto.randomUUID(), evidence);
