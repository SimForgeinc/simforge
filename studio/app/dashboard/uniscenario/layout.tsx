...
import { UniScenarioWorkspaceStatusProvider } from "./editor/status";
 ...
 *
 * `useUniScenarioWorkspaceStatus` and `useUniScenarioNotification` publish into a store, so a publisher
 * is heard from anywhere. But nothing is *rendered* unless the provider is mounted above it, and it used
 * to be mounted inside `UniScenarioEditorClient` — which meant the dataset index and the review queue
 ...
 * `dashboard/uniscenario/` gets the renderers, including ones nobody has written yet.
 *
 ...
 * doubles its backdrop. It reads as a styling bug and it is a mounting bug. `UniScenarioEditorClient`
 ...
 */
export default function UniScenarioLayout({ children }: { children: ReactNode }) {
  return <UniScenarioWorkspaceStatusProvider>{children}</UniScenarioWorkspaceStatusProvider>;
}
