...
export function useRenderingBenchmark(manifestUrl: string) {
  ...
  const quality =
    candidateIndex === null
      ? null
      : UNISCENARIO_AUTHORING_QUALITY_IDS[candidateIndex];
  ...
  const finishCandidate = useCallback(
    (result: RenderingBenchmarkResult) => {
      setState((current) => {
        ...
        if (nextIndex < UNISCENARIO_AUTHORING_QUALITY_IDS.length) {
          return {
            phase: "loading",
    ...
    (reason: unknown) => {
      ...
      setState((current) => {
        ...
        const failures = [
          ...current.failures,
          {
            quality: UNISCENARIO_AUTHORING_QUALITY_IDS[current.candidateIndex]!,
            message,
          },
        ];
        const nextIndex = current.candidateIndex + 1;
...
export function RenderingBenchmarkCard({
  ...
  manifestUrl: string;
  mapLabel: string;
  currentQuality: UniScenarioAuthoringQuality;
  onApply: (quality: UniScenarioAuthoringQuality) => void;
}) {
...
export function RenderingBenchmarkSection({
  ...
  const candidate = active
    ? choiceLabel(UNISCENARIO_AUTHORING_QUALITY_IDS[state.candidateIndex]!)
    : null;
    ...
    >
      ...
      >
        ...
        <p className="mt-2 text-sm tabular-nums text-muted-foreground">
          ...
          {UNISCENARIO_AUTHORING_QUALITY_IDS.length} complete
        </p>
        ...
        >
          {UNISCENARIO_AUTHORING_QUALITY_IDS.map((quality, index) => {
...
function choiceLabel(quality: UniScenarioAuthoringQuality) {
  return (
    UNISCENARIO_AUTHORING_QUALITY_CHOICES.find(
