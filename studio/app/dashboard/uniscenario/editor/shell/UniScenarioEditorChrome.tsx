...
export type UniScenarioEditorStatusTone =
  | "neutral"
  | "saved"
  | "working"
  | "warning"
...
const STATUS_TONE_CLASSES: Record<UniScenarioEditorStatusTone, string> = {
...
export function UniScenarioEditorChromeHeader({
  title,
  subtitle,
  badge = "UniScenario",
  status,
  ...
  badge?: ReactNode;
  status?: ReactNode;
  statusTone?: UniScenarioEditorStatusTone;
  leading?: ReactNode;
  actions?: ReactNode;
}) {
  return (
    <header
      {...headerProps}
      className={cn(
        "flex min-h-[var(--uniscenario-header-height)] min-w-0 items-center gap-3 border-b border-editor-line bg-editor-bg/95 px-3 text-editor-text backdrop-blur-md sm:px-4",
        className,
      )}
...
export function UniScenarioEditorReadout({
  children,
  className,
  ...props
