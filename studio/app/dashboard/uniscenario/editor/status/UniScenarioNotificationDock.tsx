...
import { UniScenarioNotificationCard } from "./UniScenarioNotificationCard";
 ...
 */
export function UniScenarioNotificationDock() {
  ...
  const dismiss = useUniScenarioNotificationStore((state) => state.dismiss);
  const setPaused = useUniScenarioNotificationStore((state) => state.setPaused);
  const sweep = useUniScenarioNotificationStore((state) => state.sweep);
  ...
  return (
    <div
      ...
      data-testid="uniscenario-notification-dock"
      onMouseEnter={() => setPaused(true)}
      onMouseLeave={() => setPaused(false)}
    >
      {groups.map((group) => (
        <UniScenarioNotificationCard
      ...
      {hiddenCount > 0 || expanded ? (
        <Button
          ...
          data-testid="uniscenario-notification-overflow"
          size="sm"
          variant="outline"
