...
export type UniScenarioNotificationInput = {
   ...
   * not ready" — they render in `UniScenarioBootGate` instead, as an overlay
   * over a mounted editor rather than in place of it.
   */
  blocking?: boolean;
};

export type UniScenarioNotification = UniScenarioNotificationInput & {
  detail: string | null;
  progress: number | null;
  action: NotificationAction | null;
  ttlMs: number | null;
...
export type NotificationGroup = {
  ...
  notification: UniScenarioNotification;
  count: number;
...
export function notificationTtlMs(
  input: Pick<UniScenarioNotificationInput, "severity" | "ttlMs" | "action">,
): number | null {
...
export function orderNotifications(
  notifications: Iterable<UniScenarioNotification>,
): UniScenarioNotification[] {
...
export function collapseNotifications(
  ordered: readonly UniScenarioNotification[],
): NotificationGroup[] {
...
/** The blocking entry `UniScenarioBootGate` should show, or null. */
export function resolveBlockingNotification(
  notifications: Iterable<UniScenarioNotification>,
): UniScenarioNotification | null {
  return (
...
const SOURCE_BY_KEY_PREFIX: [string, NotificationSource][] = [
  ["uniscenario-map", "map"],
  ["uniscenario-document", "scenario"],
  ["uniscenario-simulation", "simulation"],
  ["uniscenario-dataset", "dataset"],
  ["uniscenario-export", "export"],
  ["uniscenario-render", "render"],
  ["uniscenario-cosmos", "cosmos"],
  ["uniscenario-postprocess", "postprocess"],
  ["uniscenario-artifacts", "artifacts"],
  ["uniscenario-sensor", "sensor"],
  ["uniscenario-preview", "preview"],
  ["uniscenario-authoring", "authoring"],
];

/**
 * Source for a legacy `useUniScenarioWorkspaceStatus` key.
 *
