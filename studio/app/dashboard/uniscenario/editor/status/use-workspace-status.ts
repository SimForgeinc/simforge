...
import { useUniScenarioNotification } from "./use-uniscenario-notification";
import type {
  UniScenarioWorkspaceStatus,
  UniScenarioWorkspaceStatusKind,
...
/**
 * Legacy publish shape as an adapter over `useUniScenarioNotification`.
 *
 * This lives apart from `UniScenarioWorkspaceStatusProvider` on purpose. The
 ...
 *
 * @deprecated Publish with `useUniScenarioNotification` in new code — it carries
 * an explicit `source` instead of inferring one from the key.
 */
export function useUniScenarioWorkspaceStatus(
  key: string,
  status: UniScenarioWorkspaceStatus | null,
  enabled = true,
) {
