...
const STATE_VISUALS: Record<UniScenarioRenderJobState, RenderStateVisual> = {
...
export function renderStateVisual(state: UniScenarioRenderJobState): RenderStateVisual {
...
const JOB_MODE_LABELS: Record<UniScenarioRenderJobMode, string> = {
...
export function jobModeLabel(mode: UniScenarioRenderJobMode): string {
  return JOB_MODE_LABELS[mode] ?? mode;
}

/** Which gallery filter tag a managed job belongs to. */
export function jobModeTag(mode: UniScenarioRenderJobMode): "carla" | "browser" {
  return mode === "browser_render" ? "browser" : "carla";
}

export function isPostprocessMode(
  mode: UniScenarioRenderJobMode,
 ...
 * stays honest. Same trick, and same reason, as `states/UniScenarioJobDetails.tsx`.
 */
...
export function hasLiveJob(items: readonly { jobState: UniScenarioRenderJobState }[]): boolean {
...
export function artifactAvailability(
  artifact: Pick<UniScenarioRenderArtifactDto, "artifactState"> & { url?: string | null },
...
export function isPlayableVideo(
  artifact: Pick<UniScenarioRenderArtifactDto, "mediaType" | "artifactState"> & {
    url?: string | null;
  },
): boolean {
  return (
...
export function isImage(artifact: Pick<UniScenarioRenderArtifactDto, "mediaType">): boolean {
 ...
 */
export function groupArtifacts<T extends UniScenarioRenderArtifactDto & { url?: string | null }>(
 ...
 * author opens it.
 */
export function galleryItemAccessibleName(item: UniScenarioGalleryItemDto): string {
  const visual = renderStateVisual(item.jobState);
  const parts = [
...
export function postprocessSourceCandidates(
  artifacts: readonly UniScenarioPresignedArtifactDto[] | readonly (UniScenarioRenderArtifactDto & {
    url: string | null;
  })[],
): (UniScenarioRenderArtifactDto & { url: string | null })[] {
  return artifacts.filter(
    (artifact): artifact is UniScenarioRenderArtifactDto & { url: string | null } =>
