 ...
 *
 * `lib/scenario/editor/api.ts` is the editor's client and covers documents-with-content,
 ...
 * `listScenarioDocuments` parses a full `ScenarioTemplateV2` per row, which is exactly what the
 * summary projection exists to avoid.
 */

/** A typed 409 from a route that reports a name collision (`datasets`, `tags`). */
export class ScenarioNameConflict extends Error {
  constructor(
 ...
 * body is `ScenarioConflictDto`, and `refetch: true` is the server's instruction, not a hint.
 */
export class ScenarioVersionConflict extends Error {
  constructor(
    readonly currentDraftVersion: number,
    readonly current: ScenarioDocumentDto,
  ) {
...
export function resetScenarioReadCacheForTests() {
  sharedReads.clear();
}
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (
    ...
    throw new ScenarioVersionConflict(
  ...
  if (
    ...
    throw new ScenarioNameConflict(
      body.field ?? "name",
 ...
 *
 * `expectedVersion` is mandatory in `UpdateScenarioDocumentSchema` and is what makes a rename
 * safe without a lock: if the draft moved underneath, the route 409s and the caller gets a
 * `ScenarioVersionConflict` carrying the current document instead of silently overwriting an edit
...
export type ScenarioDocumentSummary = ScenarioDocumentSummaryDto;
