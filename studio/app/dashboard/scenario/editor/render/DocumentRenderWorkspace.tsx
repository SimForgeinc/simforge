...
import {
  ensureScenarioRevision,
  saveScenarioDocument,
} from "@/app/lib/scenario/editor/api";
import { getBrowserRecordingRevisionInputClient } from "@/app/lib/scenario/recording-client";
...
export function DocumentRenderWorkspace({
   ...
   *
   * Idempotent per draft version: `ensureScenarioRevision` reuses the snapshot already frozen
  ...
  const ensureSnapshot = useCallback(
    async (signal?: AbortSignal) => {
      ...
      const result = await ensureScenarioRevision({
        documentId,
        expectedDraftVersion,
        ...evidence,
        signal,
  ...
  const restoreSnapshot = useCallback(
    async (revisionId: string) => {
      ...
      const updated = await saveScenarioDocument(current, snapshot.content);
