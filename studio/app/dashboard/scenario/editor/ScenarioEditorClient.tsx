...
import {
  ...
  ScenarioVersionConflict,
  type ScenarioDocumentRecord,
...
export function ScenarioEditorClient({
  ...
  initialDocument?: ScenarioDocumentRecord;
  ...
  onSessionDocumentChange?: (document: ScenarioDocumentRecord) => void;
...
function ScenarioEditorWorkspace({
  ...
  const [record, setRecord] = useState<ScenarioDocumentRecord | null>(initialDocument ?? null);
  ...
  const recordRef = useRef<ScenarioDocumentRecord | null>(null);
  ...
  const serverRecordBySessionRef = useRef(new Map<number, ScenarioDocumentRecord | null>());
  ...
  useEffect(() => {
    ...
    Promise.all([
      listScenarioMaps(abort.signal),
      listScenarioDocuments(datasetId, abort.signal),
    ])
