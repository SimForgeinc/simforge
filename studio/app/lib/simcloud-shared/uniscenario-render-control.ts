...
export const UNISCENARIO_RENDER_RESOURCE_REQUEST_VERSION =
 ...
 */
export const UniScenarioRenderResourceRequestSchema = z.strictObject({
  schema: z.literal(UNISCENARIO_RENDER_RESOURCE_REQUEST_VERSION),
...
export type UniScenarioRenderResourceRequest = z.infer<
  typeof UniScenarioRenderResourceRequestSchema
>;

export const UNISCENARIO_PARITY_EVIDENCE_VERSION =
 ...
 * leave its path.
 */
export const UNISCENARIO_NATIVE_PHYSICS_ACCEPTANCE_LIMITS = {
  positionM: 2,
  headingDeg: 45,
  speedMps: 2,
} as const;

export const UNISCENARIO_REFERENCE_EQUIVALENCE_LIMITS = {
  positionM: 0.25,
  headingDeg: 2,
  speedMps: 0.25,
} as const;
...
export const UniScenarioParityEvidenceV1Schema = z
  .strictObject({
    schema: z.literal(UNISCENARIO_PARITY_EVIDENCE_VERSION),
...
export type UniScenarioParityEvidenceV1 = z.infer<
  typeof UniScenarioParityEvidenceV1Schema
>;

export function isUniScenarioParityEvidenceAccepted(
  evidence: UniScenarioParityEvidenceV1,
): boolean {
...
export const UniScenarioRenderHardwareProfileSchema = z.enum([
  UNISCENARIO_RTX3080_HARDWARE_PROFILE,
  UNISCENARIO_LOCAL_RTX5080_HARDWARE_PROFILE,
]);
export type UniScenarioRenderHardwareProfile = z.infer<
  typeof UniScenarioRenderHardwareProfileSchema
>;

/** Exact runtime identity submitted at registration and completion. */
export const UniScenarioRenderWorkerIdentitySchema = z.strictObject({
  workerVersion: z.string().regex(/^[a-f0-9]{40}$/),
  imageDigest: ImageDigestSchema,
  hardwareProfile: UniScenarioRenderHardwareProfileSchema,
});
export type UniScenarioRenderWorkerIdentity = z.infer<
  typeof UniScenarioRenderWorkerIdentitySchema
>;

const uniscenarioRenderControl = {
  UNISCENARIO_NATIVE_PHYSICS_ACCEPTANCE_LIMITS,
  UNISCENARIO_REFERENCE_EQUIVALENCE_LIMITS,
} as const;

export default uniscenarioRenderControl;
