...
import type { UniScenarioSimulationPreviewDto } from "./contracts";
  ...
  try {
    return await withTransaction(async (tx) => {
      ...
      if (finalization.alreadySucceeded) {
        ...
        // `uniscenario-local-artifact-retry-routes`, and why nothing below may
        // re-read the draft.
        //
...
export async function getCurrentSimulationPreview(
  context: AppContext,
  documentId: string,
): Promise<UniScenarioSimulationPreviewDto | null> {
  const rows = await queryRows<{
    artifact_id: string;
