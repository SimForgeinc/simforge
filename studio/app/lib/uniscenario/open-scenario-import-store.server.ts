...
import { withUniScenarioJobTransaction } from "./jobs/lifecycle-lock";
...
async function publishOpenScenarioImportArtifact(input: {
  ...
  return withUniScenarioJobTransaction(input.producerJobId, async (tx) => {
    ...
    const attemptId = uniscenarioId("usppat");
