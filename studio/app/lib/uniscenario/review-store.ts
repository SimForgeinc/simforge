...
import {
  ...
  type UniScenarioReviewQueueItem,
  type UniScenarioReviewQueuePage,
  type UniScenarioReviewState,
 ...
 * already have (`listUniScenarioRatingAggregates(context, documentIds)`), which cannot drive a
...
type ReviewQueueRow = {
  ...
  rating_count: number;
  review_state: UniScenarioReviewState;
...
/**
 * A single space: an ISO-8601 timestamp never contains one and `uniscenarioId()` tokens are
...
function reviewQueueItem(row: ReviewQueueRow): UniScenarioReviewQueueItem {
  return {
 ...
 */
export async function listUniScenarioReviewQueue(
  context: AppContext,
  input: { limit?: number; cursor?: string | null; datasetId?: string | null } = {},
): Promise<UniScenarioReviewQueuePage> {
  const limit = Math.max(
    1,
    Math.min(input.limit ?? UNISCENARIO_REVIEW_QUEUE_PAGE_SIZE, UNISCENARIO_REVIEW_QUEUE_MAX_PAGE_SIZE),
  );
