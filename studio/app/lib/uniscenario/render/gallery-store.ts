...
import type { UniScenarioGalleryItemDto } from "./contracts";
 ...
 * describes for `listUniScenarioDatasets`. Invalidating from the worker routes instead is possible but
...
type GalleryRow = {
  ...
  job_mode: UniScenarioGalleryItemDto["jobMode"];
  job_state: UniScenarioGalleryItemDto["jobState"];
...
function galleryItemDto(row: GalleryRow): UniScenarioGalleryItemDto {
  return {
    id: row.id,
...
export async function listRenderGallery(
  context: AppContext,
  options: { limit?: number; jobMode?: UniScenarioGalleryItemDto["jobMode"] | null } = {},
) {
