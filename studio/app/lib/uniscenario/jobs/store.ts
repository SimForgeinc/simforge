...
import type { UniScenarioJobFamily } from "./contracts";
import { settlePipelineJob, withUniScenarioJobTransaction } from "./lifecycle-lock";

type JobRow = {
  id: string;
  job_family: UniScenarioJobFamily;
  revision_id: string;
  job_type: string;
  state: string;
  priority: number;
...
export async function listOperationalJobs(
  context: AppContext,
  input: {
    family?: UniScenarioJobFamily | null;
...
export async function cancelOperationalJobWithResult(
  ...
  const current = await queryRows<{ job_family: UniScenarioJobFamily }>(
  ...
  const mutated = await withUniScenarioJobTransaction(jobId, async (tx) => {
