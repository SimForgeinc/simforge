...
export const UNISCENARIO_REVIEW_STATES = ["pending", "accepted", "rejected"] as const;

export type UniScenarioReviewState = (typeof UNISCENARIO_REVIEW_STATES)[number];

export const UniScenarioReviewQueueItemSchema = z.object({
  documentId: z.string(),
  title: z.string(),
  datasetId: z.string(),
   ...
   * and `uniscenario` carries no intention metadata at all. `summary_archetype` and
   ...
   */
  reviewState: z.enum(UNISCENARIO_REVIEW_STATES),
...
export type UniScenarioReviewQueueItem = z.infer<typeof UniScenarioReviewQueueItemSchema>;

export const UniScenarioReviewQueuePageSchema = z.object({
  items: z.array(UniScenarioReviewQueueItemSchema),
  nextCursor: z.string().nullable(),
});

export type UniScenarioReviewQueuePage = z.infer<typeof UniScenarioReviewQueuePageSchema>;

/** Matches v1's `PAGE_SIZE`, so the queue tops up at the same cadence operators are used to. */
export const UNISCENARIO_REVIEW_QUEUE_PAGE_SIZE = 12;
export const UNISCENARIO_REVIEW_QUEUE_MAX_PAGE_SIZE = 50;
