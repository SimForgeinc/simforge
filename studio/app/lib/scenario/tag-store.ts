...
import type { ScenarioTagDto } from "./contracts";
 ...
 */
export function scenarioTagSlug(label: string) {
  const slug = label
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
...
export async function listScenarioTags(context: AppContext) {
...
export type ScenarioTagWriteResult =
  | { kind: "ok"; tag: ScenarioTagDto }
  | { kind: "slug_conflict" }
  | { kind: "not_found" };
 ...
 * plainly means.
 */
export async function createScenarioTag(
  context: AppContext,
  input: { label: string; color?: string | null },
): Promise<ScenarioTagWriteResult> {
  const slug = scenarioTagSlug(input.label);
  ...
  return withTransaction(async (tx) => {
    ...
    if (!row) throw new Error("Scenario tag insert did not return a tag.");
 ...
 */
export async function updateScenarioTag(
  context: AppContext,
  tagId: string,
  input: { label?: string; color?: string | null },
): Promise<ScenarioTagWriteResult> {
 ...
 */
export async function setScenarioDocumentTags(
  context: AppContext,
  documentId: string,
  tagIds: string[],
    ...
    if (tagIds.length > 0) {
      await tx.execute(
         ...
         ON CONFLICT (document_id, tag_id) DO NOTHING`,
        {
          ...
          user_id: context.userId,
          // Same array-literal crossing as `listScenarioRatingAggregates`: the Data API has no
