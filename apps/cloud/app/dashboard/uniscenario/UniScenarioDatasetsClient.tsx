...
import { UniScenarioDatasetDetailClient } from "./[datasetId]/UniScenarioDatasetDetailClient";
import { UniScenarioEditorClient } from "./editor/UniScenarioEditorClient";
import { DatasetRenderPane } from "./render/DatasetRenderPane";
import { UniScenarioDatasetRail } from "./rail/UniScenarioDatasetRail";
import { UniScenarioIdleScene } from "./scene/UniScenarioIdleScene";
import { useUniScenarioSession } from "./scene/useUniScenarioSession";
import { UniScenarioSessionProvider } from "./scene/UniScenarioSessionContext";
import {
  UniScenarioWorldHost,
  type UniScenarioWorldState,
  type UniScenarioWorldTarget,
} from "./scene/UniScenarioWorldHost";
import * as api from "./list/api";
import { UniScenarioNameConflict } from "./list/api";
import { uniScenarioListCache } from "./list/uniScenarioListCache";
import {
  hydrateUniScenarioViewStateFromStorage,
  persistUniScenarioViewState,
} from "./list/uniScenarioViewState";
...
function datasetHref(datasetId: string) {
  return `/dashboard/uniscenario/${encodeURIComponent(datasetId)}`;
}
 ...
 * only because `dashboard/uniscenario/page.tsx` does not pass it yet — when it is absent this falls
 * back to an aborted client fetch, which is strictly worse and is meant to go away.
 */
export function UniScenarioDatasetsClient({
  initialDatasets,
}: {
  initialDatasets?: UniScenarioDatasetDto[];
}) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const routeQuery = searchParams.toString();
  const internallyWrittenQueryRef = useRef<string | null>(null);
  const [datasets, setDatasets] = useState<UniScenarioDatasetDto[] | null>(
    () =>
      initialDatasets ??
      (uniScenarioListCache.datasetsLoaded
   ...
   * than routing to `/uniscenario/editor` is what keeps the pencil a toggle instead of a one-way trip.
   */
  ...
  const [worldTarget, setWorldTarget] = useState<UniScenarioWorldTarget | null>(null);
  const [worldState, setWorldState] = useState<UniScenarioWorldState>({
    target: null,
    loadedMapVersionId: null,
    streaming: false,
    error: null,
  });
  const scenarioSession = useUniScenarioSession({
  ...
  const retainWorldTarget = useCallback((next: UniScenarioWorldTarget) => {
  ...
  const openDataset = useCallback((datasetId: string | null) => {
    const rememberedDocumentId = datasetId
      ? (uniScenarioListCache.selectedDocumentIdByDataset[datasetId] ?? null)
      : null;
  ...
  useEffect(() => {
    ...
    const preview = dataset
      ? params.get("preview")
        ...
        ?? uniScenarioListCache.selectedDocumentIdByDataset[dataset]
        ?? null
      : null;
    setPreviewDocumentId(document ?? preview);
    if (dataset && !document && preview && renderPaneRequested) {
      setDatasetRightPaneMode("render");
      const cachedDocument = uniScenarioListCache.documentsByDataset[dataset]?.find(
  ...
  const publish = useCallback((next: UniScenarioDatasetDto[]) => {
    uniScenarioListCache.datasets = next;
  ...
  useEffect(() => {
    ...
    uniScenarioListCache.datasets = initialDatasets;
    uniScenarioListCache.datasetsLoaded = true;
  }, [initialDatasets]);
  ...
  useEffect(() => {
    if (initialDatasets || uniScenarioListCache.datasetsLoaded) return;
  ...
  const orderedDatasets = useMemo(() => {
    ...
    const rank = (dataset: UniScenarioDatasetDto) =>
  ...
  const createDataset = useCallback(async () => {
    ...
    try {
      ...
      if (createError instanceof UniScenarioNameConflict) {
  ...
  const saveDatasetDetails = useCallback(async () => {
    ...
    try {
      ...
      if (updateError instanceof UniScenarioNameConflict) {
  ...
  const deleteDataset = useCallback(
    async (dataset: UniScenarioDatasetDto) => {
      if (
        !window.confirm(
      ...
      try {
        ...
        delete uniScenarioListCache.documentsByDataset[dataset.id];
        delete uniScenarioListCache.pendingDocumentsByDataset[dataset.id];
        uniScenarioListCache.loadedDatasetIds.delete(dataset.id);
        if (uniScenarioListCache.selectedDatasetId === dataset.id) {
  ...
  return (
    <UniScenarioSessionProvider session={scenarioSession}>
    <section
      className="relative h-full min-h-0 overflow-hidden bg-background text-foreground"
      data-testid="uniscenario-dataset-index"
    ...
    >
      ...
      <div
        ...
        data-testid="uniscenario-list-session"
      >
        <ResizablePanel
        ...
        >
          {openDatasetId ? (
            ...
            <UniScenarioDatasetDetailClient
      ...
      && scenarioSession.maps ? (
        <div
          ...
          data-testid="uniscenario-editor-session"
        >
          <UniScenarioEditorClient
