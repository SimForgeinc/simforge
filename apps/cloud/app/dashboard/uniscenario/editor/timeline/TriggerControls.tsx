...
import {
  CONDITION_KINDS,
  TRIGGER_KINDS,
  buildDefaultUniScenarioCondition,
  buildDefaultUniScenarioTrigger,
  numeric,
  type ConditionKind,
 ...
 *
 * Changing the kind rebuilds the whole trigger from `buildDefaultUniScenarioTrigger`
...
export function TriggerControls({
  ...
  const setKind = (next: Trigger["kind"]) =>
    commit(buildDefaultUniScenarioTrigger(next, actor, peer, references));
    ...
    <>
      <SelectMenu
        ...
        onChange={(next) =>
          onChange({
            ...value,
            condition: buildDefaultUniScenarioCondition(
            ...
            <SelectMenuField
              ...
              onChange={(kind) => onChange({
                ...
                operands: value.operands.map((item, at) => at === index
                  ? buildDefaultUniScenarioCondition(kind as LeafCondition["kind"], fallbackActor, fallbackPeer) as LeafCondition
                  : item),
              })}
              className="h-8 text-xs"
            />
    ...
    return (
      <fieldset className="space-y-2 border border-border bg-muted/20 p-2">
        ...
        <SelectMenuField
          ...
          onChange={(kind) => onChange({
            ...value,
            operand: buildDefaultUniScenarioCondition(kind as LeafCondition["kind"], fallbackActor, fallbackPeer) as LeafCondition,
          })}
          className="h-8 text-xs"
        />
        <ConditionControls
