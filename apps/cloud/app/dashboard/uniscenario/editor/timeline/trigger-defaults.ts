import type { Condition, Interaction, Trigger } from "@uniscenarios/scenario-model";
 ...
 */
export function buildDefaultUniScenarioTrigger(
  kind: Trigger["kind"],
  actor: string,
  peer: string,
  ...
  return {
    kind,
    condition: buildDefaultUniScenarioCondition("speed", actor, peer),
    byLatest: 10,
    ifNever: "skip",
  };
}

export function buildDefaultUniScenarioCondition(
  kind: ConditionKind,
  actor: string,
  peer: string,
): Condition {
  switch (kind) {
    ...
    case "and":
      return {
        kind,
        operands: [
          buildDefaultUniScenarioCondition("speed", actor, peer),
          buildDefaultUniScenarioCondition("distance", actor, peer),
        ] as [never, never],
      };
    case "or":
      return {
        ...
        operands: [
          buildDefaultUniScenarioCondition("speed", actor, peer),
          buildDefaultUniScenarioCondition("standstill", actor, peer),
        ] as [never, never],
      };
    case "not":
      return {
        kind,
        operand: buildDefaultUniScenarioCondition("collision", actor, peer) as never,
      };
  }
}
