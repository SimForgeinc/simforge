...
import type { UniScenarioArtifactDto } from "@/app/lib/uniscenario/contracts";

/**
 * Resolve a gallery tile's preview URL, but only once the tile is near the viewport.
 *
 * `UniScenarioGalleryItemDto` carries `previewArtifactId` and not a URL, deliberately: a presigned URL
...
export function useArtifactPreviewUrl(artifactId: string | null, enabled: boolean) {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/uniscenario/artifacts/${encodeURIComponent(artifactId)}`, {
      ...
      .then((response) => (response.ok ? (response.json() as Promise<UniScenarioArtifactDto>) : null))
      .then((artifact) => {
