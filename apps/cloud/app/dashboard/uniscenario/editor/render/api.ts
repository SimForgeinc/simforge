 ...
 *
 * Kept beside the components rather than in `app/lib/uniscenario/editor/api.ts` because that module
 * is shared with every other port lane and none of them need these seven endpoints. Same reason
 * `lib/uniscenario/render/contracts.ts` is its own file.
 *
...
import type {
  ...
  UniScenarioPostprocessInput,
  UniScenarioPresignedArtifactDto,
  UniScenarioRenderArtifactDto,
  UniScenarioRenderJobDetailDto,
  UniScenarioRenderJobMode,
  UniScenarioExportInspectionDto,
} from "@/app/lib/uniscenario/render/contracts";
import type {
  UniScenarioExportDto,
  UniScenarioRenderJobDto,
} from "@/app/lib/uniscenario/contracts";
import type { SubmitUniScenarioRenderIntent } from "@/app/lib/uniscenario/render-wire-contracts";
...
export type PresignedArtifact =
  | UniScenarioPresignedArtifactDto
  | (UniScenarioRenderArtifactDto & { url: null });
 ...
 */
export type ArtifactMetadata = UniScenarioRenderArtifactDto & {
  url?: undefined;
  renderJobId?: string | null;
};

export type WorkspaceArtifact = UniScenarioRenderArtifactDto & { renderJobId: string | null };
...
export function fetchRenderGallery(
  options: {
    ...
    jobMode?: UniScenarioRenderJobMode | null;
    limit?: number;
  },
  signal?: AbortSignal,
): Promise<{ items: UniScenarioGalleryItemDto[]; hiddenCount: number }> {
...
export function fetchRenderJobDetail(
  jobId: string,
  signal?: AbortSignal,
): Promise<UniScenarioRenderJobDetailDto> {
...
export async function fetchPostprocessChildren(
  ...
  signal?: AbortSignal,
): Promise<UniScenarioGalleryItemDto[]> {
  const body = await request<{ items: UniScenarioGalleryItemDto[] }>(
...
export function createPostprocessJob(
  input: UniScenarioPostprocessInput,
...
export async function listOpenScenarioExports(
  ...
  const result = await request<{ exports: UniScenarioExportDto[] }>(
...
export function inspectOpenScenarioExport(
  exportId: string,
  signal?: AbortSignal,
): Promise<UniScenarioExportInspectionDto> {
...
export type UniScenarioValidationRunDto = {
  id: string;
  revision_id: string;
...
export async function listValidationRuns(
  revisionId: string,
  signal?: AbortSignal,
): Promise<UniScenarioValidationRunDto[]> {
  const body = await request<{ validationRuns: UniScenarioValidationRunDto[] }>(
