...
import type { MaterializedTrafficArtifactEnvelope } from "@uniscenarios/sim-engine";
...
import { allSumoSignalsGreenFromExtensions } from "@uniscenarios/ambient-traffic";
...
import { firstEnabledDashCamera } from "@uniscenarios/scenario-model";
...
export function SumoPreviewTraffic({
  ...
  const externalActors = (playback.controller?.currentActors ?? []).map((sampled) => {
    ...
    // is recorded facing due east. See `restingHeading` in `@uniscenarios/playback`.
