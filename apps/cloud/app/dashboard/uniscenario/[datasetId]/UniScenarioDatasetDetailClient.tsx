...
import { UniScenarioDocumentCreator } from "../list/UniScenarioDocumentCreator";
import { UniScenarioMapPickerDialog } from "../list/UniScenarioMapPickerDialog";
import { UniScenarioTagFilterDropdown } from "../list/UniScenarioTagFilterDropdown";
...
import {
  ...
  rememberUniScenarioSelection,
} from "../list/uniScenarioViewState";
import { useUniScenarioDocumentActions } from "../list/useUniScenarioDocumentActions";
import { useUniScenarioDocumentList } from "../list/useUniScenarioDocumentList";
import { useUniScenarioOpenScenarioImport } from "../list/useUniScenarioOpenScenarioImport";
import { useUniScenarioTagManager } from "../list/useUniScenarioTagManager";
 ...
 *
 * This surface did not exist in v2 at all: the editor called `listUniScenarioDocuments(datasetId)` and
 ...
 */
export function UniScenarioDatasetDetailClient({
  datasetId,
  initialDataset,
  initialDocuments,
  onBack,
  ...
  datasetId: string;
  initialDataset?: UniScenarioDatasetDto | null;
  initialDocuments?: UniScenarioDocumentSummaryDto[];
  /**
  ...
  onPreviewDocument?: (document: UniScenarioDocumentSummaryDto) => void;
  onExitEdit?: () => void;
  ...
  const [dataset, setDataset] = useState<UniScenarioDatasetDto | null>(
    () =>
      initialDataset ??
      uniScenarioListCache.datasets.find((entry) => entry.id === datasetId) ??
      null,
  );
  ...
  const list = useUniScenarioDocumentList(datasetId);
  ...
  useEffect(() => {
    ...
    // mounted-but-hidden behind the editor rather than unmounting (see UniScenarioDatasetsClient).
    //
  ...
  const tagManager = useUniScenarioTagManager({
    datasetId,
  ...
  // `resolveUniScenarioDatasetAccess` is the enforcement; this is the display hint that keeps the
  ...
  const openDocument = useCallback(
    (document: UniScenarioDocumentSummaryDto) => {
      rememberUniScenarioSelection(datasetId, document.id);
      router.push(editorHref(datasetId, document.id));
    },
    [datasetId, router],
  );

  const openScenarioImport = useUniScenarioOpenScenarioImport({
    datasetId,
    maps,
    onImported: (document) => {
  ...
  return (
    <>
      ...
      <section
        ...
        data-testid="uniscenario-document-index"
        data-dataset-id={datasetId}
      >
        {showTopBarControls ? (
          <header
            className="space-y-2.5 border-b border-white/15 px-3 py-3"
            data-testid="uniscenario-scenario-list-header"
          >
        ...
        <div className={cn("min-h-0 flex-1 overflow-hidden")}>
          <UniScenarioDocumentCreator
            ...
            onPersistViewState={persistUniScenarioViewState}
            activeDocumentId={
              uniScenarioListCache.selectedDocumentIdByDataset[datasetId] ??
              null
            }
