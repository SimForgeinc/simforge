...
import {
  ...
  UniScenarioReviewQueuePageSchema,
...
async function fetchQueuePage(cursor?: string | null) {
  const params = new URLSearchParams({ limit: String(UNISCENARIO_REVIEW_QUEUE_PAGE_SIZE) });
  ...
  return UniScenarioReviewQueuePageSchema.parse(body);
}

export function UniScenarioReviewQueue() {
  useSetPageTitle("Scenario Review");
  const cardRefs = useRef(new Map<string, HTMLElement>());
  const [items, setItems] = useState<UniScenarioReviewQueueItem[]>([]);
        ...
        actions={
          <>
            <Button asChild variant="outline" size="sm">
              <Link href="/dashboard/uniscenario">Datasets</Link>
            </Button>
            <Button
              type="button"
              variant="outline"
