...
import type { UniScenarioTagDto } from "@/app/lib/uniscenario/contracts";
...
export type UniScenarioCreatorFilterOption = {
  value: string;
  label: string;
  count: number;
};

export function UniScenarioTagFilterDropdown({
  tags,
  creatorOptions,
  selectedTagFilter,
  ...
  creatorOptions: UniScenarioCreatorFilterOption[];
      ...
      <DropdownMenuContent align="end" className="min-w-[220px]">
        ...
        {creatorOptions.length === 0 ? (
          ...
          creatorOptions.map((creator) => (
            <DropdownMenuItem
              key={creator.value}
              data-uniscenario-filter-creator={creator.value}
              onSelect={() => {
        ...
        {tags.length === 0 ? (
          ...
          tags.map((tag) => (
            <DropdownMenuItem
              key={tag.id}
              data-uniscenario-filter-tag-id={tag.id}
              onSelect={() => {
