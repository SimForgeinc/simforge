...
export class UniScenarioImportError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "UniScenarioImportError";
  }
}
...
export function buildDocumentTransferFile(document: {
  ...
  mapVersionId: string | null;
  content: ScenarioTemplateV2;
}): UniScenarioDocumentTransferFile {
  return {
    simforgeUniScenarioExport: 1,
...
export function readDocumentTransferFile(input: unknown): {
  ...
  if (!isRecord(input)) {
    throw new UniScenarioImportError("Scenario JSON must be a JSON object.");
  }
  ...
  try {
    ...
    throw new UniScenarioImportError(
      `That file is not a valid UniScenario template: ${
