 ...
 *
 * `thumbnailUrl` resolves `map_versions.thumbnail_artifact_id` through a stable UniScenario route.
 * The browser never reads the source map-assets bucket or guesses a storage key.
 */
export type UniScenarioMapOption = {
  mapVersionId: string;
...
export type UniScenarioMapGroup = {
...
export function groupDocumentsByMap(
  ...
  const mapByLabel = new Map<string, UniScenarioMapOption>();
  ...
  const groups = new Map<string, UniScenarioMapGroup>();
...
export function groupVariationsBySource(
  ...
  const bySource = new Map<string, UniScenarioDocumentSummaryDto[]>();
  for (const document of documents) {
