...
import {
  ...
  UniScenarioRendererEngineSchema,
} from "@/app/lib/uniscenario/render-wire-contracts";
import { listRenderJobArtifacts } from "./artifact-store";
import type {
  UniScenarioJobEventDto,
  UniScenarioRenderAttemptDto,
...
type DetailRow = {
  ...
  job_mode: UniScenarioRenderJobDetailDto["jobMode"];
  job_state: UniScenarioRenderJobDetailDto["jobState"];
...
export async function listRenderJobEvents(
  ...
  return rows.map((row): UniScenarioJobEventDto => {
...
export async function getRenderJobDetail(
  context: AppContext,
  jobId: string,
): Promise<UniScenarioRenderJobDetailDto | null> {
  ...
  return {
    ...
    rendererEngine: job.renderer_engine
      ? UniScenarioRendererEngineSchema.parse(job.renderer_engine)
      : null,
