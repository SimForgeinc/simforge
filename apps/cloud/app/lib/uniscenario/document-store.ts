...
import {
  ...
  type UniScenarioMaterializedTrafficReference,
  OPENSCENARIO_NATIVE_PROFILE,
  type UniScenarioDocumentDto,
  type UniScenarioDocumentSummaryDto,
  type UniScenarioDocumentSummaryPageDto,
  type UniScenarioMapDescriptorDto,
  type UniScenarioRevisionDto,
} from "./contracts";
import { canonicalContentSha256, uniscenarioId } from "./core";
...
type DocumentRow = {
  ...
  dataset_id: string;
  authoring_quality_id: UniScenarioDocumentDto["authoringQualityId"];
  created_at: string;
  updated_at: string;
...
type RevisionRow = {
  ...
  export_state: UniScenarioRevisionDto["export"]["status"];
...
function documentDto(row: DocumentRow): UniScenarioDocumentDto {
  return {
    id: row.id,
...
function revisionDto(row: RevisionRow): UniScenarioRevisionDto {
...
export async function listUniScenarioDocuments(context: AppContext, limit = 50, datasetId?: string | null) {
...
type DocumentSummaryRow = {
  ...
  derivation_kind: UniScenarioDocumentSummaryDto["derivationKind"];
...
function documentSummaryDto(row: DocumentSummaryRow): UniScenarioDocumentSummaryDto {
  return {
    id: row.id,
    workspaceId: row.workspace_id,
    title: row.title,
    ...
    tags: jsonColumn<UniScenarioDocumentSummaryDto["tags"]>(row.tags, []),
...
export async function listUniScenarioDocumentSummaries(
  context: AppContext,
  input: { datasetId: string; limit?: number; cursor?: string | null },
): Promise<UniScenarioDocumentSummaryPageDto> {
 ...
 */
export async function duplicateUniScenarioDocument(
  context: AppContext,
  documentId: string,
  input: { title?: string; datasetId?: string } = {},
): Promise<{ kind: "created"; document: UniScenarioDocumentDto } | { kind: "not_found" }> {
  return withTransaction(async (tx) => {
    ...
    const copyId = uniscenarioId("uscn");
    ...
    if (!created) throw new Error("UniScenario document duplicate did not return a document.");
    return { kind: "created" as const, document: documentDto(created) };
  });
}

export async function getUniScenarioDocument(context: AppContext, documentId: string) {
  const rows = await queryRows<DocumentRow>(
    `${DOCUMENT_SELECT}
     WHERE d.workspace_id = :workspace_id AND d.id = :document_id AND d.deleted_at IS NULL
     LIMIT 1`,
...
export async function createUniScenarioDocument(
  context: AppContext,
  input: {
    title: string;
    description?: string;
    ...
    datasetId: string;
    authoringQualityId: UniScenarioDocumentDto["authoringQualityId"];
  },
) {
  const documentId = uniscenarioId("uscn");
  ...
  return withTransaction(async (tx) => {
    ...
    if (!row) throw new Error("UniScenario document insert did not return a document.");
    return documentDto(row);
  });
}

export async function updateUniScenarioDocument(
  context: AppContext,
  documentId: string,
  input: {
...
): Promise<{ kind: "updated"; document: UniScenarioDocumentDto } | { kind: "conflict"; current: UniScenarioDocumentDto } | { kind: "not_found" }> {
  return withTransaction(async (tx) => {
    ...
    if (!updated) throw new Error("UniScenario document disappeared during update.");
    return { kind: "updated" as const, document: documentDto(updated) };
  });
}

export async function softDeleteUniScenarioDocument(context: AppContext, documentId: string) {
...
export async function createUniScenarioRevision(
  context: AppContext,
  documentId: string,
  input: { expectedVersion: number; idempotencyKey?: string; ambient: UniScenarioAmbientProvenance; materializedTraffic?: UniScenarioMaterializedTrafficReference },
) {
  return withTransaction(async (tx) => {
    ...
    if (existingDraftRevision) {
      ...
      const retryExportId = uniscenarioId("usexp");
      await tx.execute(
      ...
      if (!retried) throw new Error("UniScenario export retry did not return a revision.");
    ...
    const revisionId = uniscenarioId("usrev");
    const exportId = uniscenarioId("usexp");
    ...
    if (!created) throw new Error("UniScenario revision insert did not return a revision.");
...
export async function listUniScenarioRevisions(context: AppContext, documentId: string) {
...
export async function getUniScenarioRevision(context: AppContext, revisionId: string) {
  const rows = await queryRows<RevisionRow>(
    revisionSelect("r.workspace_id = :workspace_id AND r.id = :revision_id"),
    { workspace_id: context.workspaceId, revision_id: revisionId },
  );
...
export type UniScenarioMapBrowserAsset = {
  bucket: string;
  key: string;
 ...
 * the `use cache` rules in `plans/2026-08-04-uniscenario-v2-parity-plan.md`
 ...
 */
async function readUniScenarioMapBrowserAsset(
  mapVersionId: string,
  relativePath: string,
): Promise<UniScenarioMapBrowserAsset | null> {
  "use cache";
  cacheLife("days");
  cacheTag(`uniscenario:map-version:${mapVersionId}`);
...
export async function getUniScenarioMapBrowserAsset(
  _context: AppContext,
  mapVersionId: string,
  relativePath: string,
): Promise<UniScenarioMapBrowserAsset | null> {
  return readUniScenarioMapBrowserAsset(mapVersionId, relativePath);
}

export type UniScenarioMapBrowserAssetRequest = {
  mapVersionId: string;
  relativePath: string;
};

export async function getUniScenarioMapBrowserAssets(
  _context: AppContext,
  requests: UniScenarioMapBrowserAssetRequest[],
): Promise<Array<UniScenarioMapBrowserAssetRequest & UniScenarioMapBrowserAsset>> {
  if (requests.length === 0) return [];
  return queryRows<
    UniScenarioMapBrowserAssetRequest & UniScenarioMapBrowserAsset
  >(
    `WITH requested AS (
...
export type UniScenarioBrowserCacheAsset = {
  relativePath: string;
  sha256: string;
  byteLength: number;
  mediaType: string;
...
export type UniScenarioBrowserCacheMap = {
  mapVersionId: string;
  closureSha256: string;
  assets: UniScenarioBrowserCacheAsset[];
};
 ...
 */
export async function listUniScenarioBrowserCacheInventory(
  _context: AppContext,
): Promise<{ releaseKey: string; maps: UniScenarioBrowserCacheMap[] }> {
  ...
  const maps = new Map<string, UniScenarioBrowserCacheMap>();
...
/**
 * The cacheable half of {@link listUniScenarioMapDescriptors}: storage
...
async function readUniScenarioMapDescriptorRows(_activeReleaseCacheKey: string) {
  "use cache";
  cacheLife("days");
  cacheTag("uniscenario:maps:global");
...
export async function listUniScenarioMapDescriptors(_context: AppContext) {
  ...
  const rows = await readUniScenarioMapDescriptorRows(activeReleaseCacheKey);
  const thumbnailMapVersionIds = new Set(
    (await readAvailableThumbnailMapVersionIds()).map((row) => row.id),
  );
  return rows.map((row): UniScenarioMapDescriptorDto => {
