...
type RatingRow = {
  ...
  score: number;
  comment: string | null;
  reviewed_via: UniScenarioDocumentRatingDto["reviewedVia"];
  created_at: string;
  updated_at: string;
};
...
type AggregateRow = {
  ...
  review_state: UniScenarioRatingAggregateDto["reviewState"];
  viewer_score: number | null;
};

function ratingDto(row: RatingRow): UniScenarioDocumentRatingDto {
  return {
...
function aggregateDto(row: AggregateRow): UniScenarioRatingAggregateDto {
  return {
    documentId: row.document_id,
 ...
 */
export async function upsertUniScenarioDocumentRating(
  context: AppContext,
  documentId: string,
  input: {
    score: number;
    comment?: string | null;
    reviewedVia: UniScenarioDocumentRatingDto["reviewedVia"];
...
export async function deleteUniScenarioDocumentRating(
  context: AppContext,
  documentId: string,
) {
...
export async function getUniScenarioRatingAggregate(
  context: AppContext,
  documentId: string,
): Promise<UniScenarioRatingAggregateDto | null> {
 ...
 */
export async function listUniScenarioRatingAggregates(
  context: AppContext,
  documentIds: string[],
): Promise<UniScenarioRatingAggregateDto[]> {
  ...
  const rows = await queryRows<AggregateRow>(
     ...
     ORDER BY v.document_id`,
    {
      ...
      // is cast server-side. Ids are opaque `uniscenarioId()` tokens, and this is still a bound
