...
import type { MaterializedTrafficArtifactEnvelope, SceneTrace } from "@uniscenarios/sim-engine";
import {
...
import type { UniScenarioMapEntry } from "@uniscenarios/editor-core";
...
export type UniScenarioDocumentRecord = UniScenarioDocumentDto;
export type UniScenarioExportRecord = UniScenarioExportDto;

export class UniScenarioVersionConflict extends Error {
  constructor(readonly currentVersion: number | null) {
    super("This UniScenario changed in another session. Reload before saving again.");
  }
}
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (response.status === 409) {
    const conflict = await response.json().catch(() => null) as UniScenarioConflictDto | null;
    throw new UniScenarioVersionConflict(conflict?.currentDraftVersion ?? null);
  }
  if (!response.ok) {
    const body = await response.json().catch(() => null) as { error?: string; message?: string } | null;
    throw new Error(body?.message ?? body?.error ?? `UniScenario request failed (${response.status})`);
  }
...
export async function listUniScenarioMaps(signal?: AbortSignal): Promise<UniScenarioMapEntry[]> {
  const response = await request<UniScenarioMapDescriptorDto[] | { maps?: UniScenarioMapDescriptorDto[] }>(
...
export async function listUniScenarioDocuments(datasetId: string, signal?: AbortSignal): Promise<UniScenarioDocumentRecord[]> {
  const response = await request<UniScenarioDocumentRecord[] | { items?: UniScenarioDocumentRecord[]; documents?: UniScenarioDocumentRecord[] }>(
...
export function createUniScenarioDocument(
  input: {
    title: string;
  ...
  /** See `saveUniScenarioDocument` — the first save of a new document can also
   * be the one racing an unload. */
  options: { keepalive?: boolean } = {},
): Promise<UniScenarioDocumentRecord> {
...
export function saveUniScenarioDocument(
  document: UniScenarioDocumentRecord,
  content: ScenarioTemplateV2,
  title = document.title,
  authoringQualityId: UniScenarioAuthoringQuality = document.authoringQualityId,
  /**
...
export function createUniScenarioRevision(
  document: UniScenarioDocumentRecord,
  idempotencyKey = crypto.randomUUID(),
  evidence?: {
    ambient: UniScenarioAmbientProvenance;
    materializedTraffic: UniScenarioMaterializedTrafficReference;
  },
): Promise<CreateUniScenarioRevisionResultDto> {
  if (!evidence) {
...
export async function uploadUniScenarioMaterializedTraffic(
  document: Pick<UniScenarioDocumentRecord, "id" | "draftVersion">,
  envelope: MaterializedTrafficArtifactEnvelope,
  options: { readonly signal?: AbortSignal } = {},
): Promise<UniScenarioMaterializedTrafficReference> {
  const identity = {
  ...
  return request<UniScenarioMaterializedTrafficReference>(
 ...
 */
export async function uploadAndConsumeUniScenarioMaterializedTraffic(
  document: Pick<UniScenarioDocumentRecord, "id" | "draftVersion">,
  envelope: MaterializedTrafficArtifactEnvelope,
  trace: SceneTrace,
  ...
  readonly reference: UniScenarioMaterializedTrafficReference;
  readonly evidence: BrowserMaterializedTrafficSceneTraceEvidence;
}> {
  const reference = await uploadUniScenarioMaterializedTraffic(document, envelope, options);
  const descriptor = await request<UniScenarioArtifactDto>(
...
function revisionResult(revision: UniScenarioRevisionDto): CreateUniScenarioRevisionResultDto {
  return {
 ...
 */
export async function ensureUniScenarioRevision(input: {
  documentId: string;
  ...
  const document = input.expectedDraftVersion == null
    ? await request<UniScenarioDocumentRecord>(
  ...
  const response = await request<{ revisions: UniScenarioRevisionDto[] }>(
...
): Promise<UniScenarioExportRecord> {
  const attempts = options.attempts ?? 120;
  const intervalMs = options.intervalMs ?? 1_000;
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    const response = await request<UniScenarioExportDto[] | { exports?: UniScenarioExportDto[] }>(
...
export async function downloadExportArtifact(artifactId: string): Promise<void> {
  const artifact = await request<UniScenarioArtifactDto>(
...
export async function openUniScenarioArtifact(artifactId: string): Promise<void> {
  const artifact = await request<UniScenarioArtifactDto>(
...
export function submitRevisionJob(
  ...
  renderSpec?: UniScenarioRenderSpec,
): Promise<UniScenarioRenderJobDto> {
...
export function renderRevision(revisionId: string, executionPackageId: string, renderSpec: UniScenarioRenderSpec) {
  return submitRevisionJob(revisionId, executionPackageId, "full_render", renderSpec);
}

export async function listUniScenarioRenderJobs(): Promise<UniScenarioRenderJobDto[]> {
  const result = await request<{ renderJobs: UniScenarioRenderJobDto[] }>("/api/uniscenario/render-jobs");
  return result.renderJobs;
}

export async function cancelUniScenarioRenderJob(id: string): Promise<UniScenarioRenderJobDto> {
  return request(`/api/uniscenario/render-jobs/${encodeURIComponent(id)}`, { method: "DELETE" });
}

export function getUniScenarioJobProvenance(id: string): Promise<UniScenarioJobProvenanceDto> {
  return request(`/api/uniscenario/render-jobs/${encodeURIComponent(id)}/provenance`);
}

export { UNISCENARIO_SCHEMA_VERSION };
