...
import { AUTHORING_QUALITY } from "@/app/dashboard/uniscenario/editor/authoring-quality";
import {
...
} from "@/app/lib/uniscenario/sumo-runtime";
...
function planCacheRequest(map: UniScenarioMapOption, profile: RenderingPreference): Request {
...
function batchDownloadRequest(asset: ProfileMapAsset) {
  ...
  const match = /^\/api\/uniscenario\/maps\/([^/]+)\/browser-assets\/(.+)$/.exec(
    url.pathname,
  );
...
export async function createProfileMapPlan(
  maps: readonly UniScenarioMapOption[],
  profile: RenderingPreference,
  signal: AbortSignal,
