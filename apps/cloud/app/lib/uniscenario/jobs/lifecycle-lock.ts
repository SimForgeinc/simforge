...
/**
 * Every UniScenarios lifecycle writer takes this transaction-scoped lock before
 ...
 */
export const UNISCENARIO_JOB_LOCK_SQL =
  "SELECT pg_advisory_xact_lock(hashtext('uniscenario-job:' || :job_id)) AS locked";

export async function lockUniScenarioJob(tx: JobTransaction, jobId: string) {
  await tx.execute(UNISCENARIO_JOB_LOCK_SQL, { job_id: jobId });
}

export async function withUniScenarioJobTransaction<T>(
  jobId: string,
  fn: (tx: JobTransaction) => Promise<T>,
) {
  return withTransaction(async (tx) => {
    await lockUniScenarioJob(tx, jobId);
    return fn(tx);
  });
}
 ...
 */
export async function claimFirstEligibleUniScenarioJob<TCandidate, TClaim>(
  candidates: readonly TCandidate[],
  jobId: (candidate: TCandidate) => string,
  claim: (tx: JobTransaction, candidate: TCandidate) => Promise<TClaim | null>,
) {
  for (const candidate of candidates) {
    const result = await withUniScenarioJobTransaction(jobId(candidate), (tx) =>
      claim(tx, candidate),
    );
...
export async function settlePipelineJob(
  ...
  const params = {
    ...
    request_seed: uniscenarioId("prr"),
  };
    ...
    `WITH matched AS (
       ...
       UPDATE pipeline_run_items item
          SET stage_status = eligible.next_status,
              ...
              END,
              error_message = CASE
                WHEN :stage_status = 'failed' THEN 'Canonical UniScenarios child job failed.'
                WHEN :stage_status = 'cancelled' THEN 'Canonical UniScenarios child job was cancelled.'
