...
import { sha256, uniscenarioId } from "../core";
import type { UniScenarioCpuJobFamily, UniScenarioJobFamily } from "./contracts";
import {
...
type CpuAttemptFamily = Exclude<UniScenarioJobFamily, "openscenario_compile">;
...
async function insertCpuEvent(
  tx: JobTransaction,
  input: { workspaceId: string; jobFamily: UniScenarioJobFamily; jobId: string; attemptId?: string | null; type: string; payload?: Record<string, unknown> },
) {
  await tx.execute(
...
async function expireCpuAttempts() {
  ...
  for (const candidate of candidates) {
    await withUniScenarioJobTransaction(candidate.job_id, async (tx) => {
...
async function claimValidationOrPostprocess(input: {
  workerId: string;
  leaseSeconds: number;
  families: readonly UniScenarioCpuJobFamily[];
}) {
  ...
  const claimed = await claimFirstEligibleUniScenarioJob(
    candidates,
    (candidate) => candidate.job_id,
    async (tx, candidate) => {
      const fenceToken = randomBytes(32).toString("hex");
      const attemptId = uniscenarioId("uscat");
    ...
    if (candidate.job_family === "openscenario_render") {
      source = await tx.queryOne<BrowserRenderSource>(
        `SELECT 'openscenario_render'::text AS job_family, j.id AS job_id, j.workspace_id,
                ...
                r.document_id, d.dataset_id,
                j.render_spec::text AS render_spec, j.render_spec_sha256, j.attempt_count
           FROM uniscenario.render_jobs j
  ...
  if (claimed.source.job_family === "openscenario_render") {
    ...
    return {
      ...
      payload: {
        ...
        renderSpec: parseJsonObject(source.render_spec),
        renderSpecSha256: source.render_spec_sha256,
        recordingInputUrl: `${
          process.env.UNISCENARIO_API_BASE_URL?.replace(/\/$/, "") ||
          "http://127.0.0.1:5199"
        }/api/uniscenario/revisions/${source.revision_id}/recording-input`,
      },
    };
  }
...
export async function heartbeatCpuJob(
  jobId: string,
  input: {
    jobFamily: UniScenarioJobFamily;
    attemptId: string;
    fenceToken: string;
    leaseSeconds: number;
    progress?: number;
  ...
  return withUniScenarioJobTransaction(jobId, async (tx) => {
    ...
    const state = await tx.queryRows<{ cancel_requested: boolean }>(
      `UPDATE uniscenario.${table} SET progress = GREATEST(progress, :progress), updated_at = NOW()
...
export async function reserveCpuJobOutputs(
  ...
  const reserved = await withUniScenarioJobTransaction(jobId, async (tx) => {
...
export async function failCpuJob(
  ...
  const result = await withUniScenarioJobTransaction(jobId, async (tx) => {
    ...
    await tx.execute(
      `UPDATE uniscenario.${table} SET ${stateColumn} = :state,
