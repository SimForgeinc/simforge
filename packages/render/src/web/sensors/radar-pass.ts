...
import { captureIdPass, decodeRgb24Ids, type IdLegend } from './id-pass.js';
import type { RadarDetection } from './csv.js';
import { CubeCameraPool, unprojectCubeDepth, type CubeFaceName, type DepthCube } from './lidar-pass.js';
import type { RenderResourcePool } from './render-targets.js';

export type InstanceIdCube = Readonly<Partial<Record<CubeFaceName, Readonly<{
  width: number;
  height: number;
  ids: Uint32Array;
}>>>>;
...
export function captureInstanceIdCube(input: Readonly<{
  ...
  const faces: Partial<Record<CubeFaceName, NonNullable<InstanceIdCube[CubeFaceName]>>> = {};
...
export function captureRadarFrame(input: Readonly<{
  ...
  for (let sample = 0; sample < count; sample += 1) {
    ...
    const depthFace = input.faces[hit.face];
    if (!idFace || !depthFace || idFace.width !== depthFace.width || idFace.height !== depthFace.height || idFace.ids.length !== idFace.width * idFace.height) {
