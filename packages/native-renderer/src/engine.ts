 ...
 * binary (native/render-core), and repackages its raw pass outputs into
...
import { parseRenderIntent, type RenderIntentV1 } from '@uniscenarios/scenario-model';
...
export function resolveBinary(options: NativeRenderEngineOptions): string {
  ...
  const candidates = [
    path.resolve('native/target/release/native-render-job'),
    path.resolve('native/target/debug/native-render-job'),
  ];
