...
export interface Corpus {
  ...
  readonly generator: 'packages/evaluation tools/build-corpus.ts';
...
async function main(): Promise<void> {
  ...
  const corpus: Corpus = {
    ...
    generator: 'packages/evaluation tools/build-corpus.ts',
    scenarios,
  };
  const out = path.join(REPO_ROOT, 'packages/evaluation/fixtures/corpus.v1.json');
