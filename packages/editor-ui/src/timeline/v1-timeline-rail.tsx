...
import { newTemplateId, type Interaction, type ReasoningTraceSegment } from "@uniscenarios/scenario-model";
...
import type { SignalTimelineBand } from "./signal-band";
import {
  choreographyWindow,
  rangePercent,
...
import { indicationLabel, indicationSwatch } from "./indication-style";
import {
  OBJECT_CATALOG_IDS,
  ObjectCatalogIcon,
  type ObjectCatalogId,
} from "../catalog/object-catalog-icon";
import {
  PEDESTRIAN_CATALOG_IDS,
  PedestrianCatalogIcon,
  type PedestrianCatalogId,
} from "../catalog/pedestrian-catalog-icon";
import {
  VEHICLE_CATALOG_IDS,
  VehicleCatalogIcon,
  type VehicleCatalogId,
} from "../catalog/vehicle-catalog-icon";
import { DynamicActorCatalogIcon, isDynamicActorCatalogId } from '../catalog/dynamic-actor-catalog-icon';
import { cn } from "../cn";
import { isUnconfiguredSimpleTimedRoute } from "../simple-route-status";
import { TimelineCarlaCompatibilityMarker } from "./timeline-carla-compatibility-marker";
import { TimelineRuler } from "./timeline-ruler";
import { TimelineTransportControls } from "./timeline-transport-controls";
import {
  buildTimelineCues,
  timelineCauseLabel,
...
} from "./timeline-glass-surface";
import {
  type TrafficLightAuthoring,
} from "./traffic-light-authoring";
import { EditorDetailsPanel } from "../inspector/editor-details-panel";
import { CanonicalInteractionComposer } from "./canonical-interaction-composer";
import {
  AlertTriangle,
  Box,
  BrainCircuit,
...
export function V1TimelineRail({
  ...
  return (
    <section
      ...
      data-testid="uniscenario-timeline-dock"
    ...
    >
      <>
          ...
          <div
            ...
            className="ueui-timeline-height-handle"
            ...
            tabIndex={0}
          >
            <span className="ueui-timeline-height-grip" />
          </div>
          <div
            ...
            className="ueui-timeline-split-handle"
            ...
            tabIndex={0}
          >
            <span className="ueui-timeline-split-line" />
          </div>
          <header
            className="ueui-timeline-header"
            data-testid="timeline-topbar"
          >
            <div className="ueui-timeline-header-identity">
              <div
                className="ueui-timeline-identity-content ueui-timeline-identity-content-column"
                {...{ [IDENTITY_CONTENT_ATTR]: "" }}
              >
                <strong className="ueui-timeline-header-title">
                  Timeline
                </strong>
            ...
            <div
              className="ueui-timeline-ruler-slot"
            ...
            >
              <TimelineRuler
                ...
                className="ueui-timeline-ruler-inline"
              />
            </div>
          </header>

          <div
            className="ueui-timeline-playhead"
          ...
          >
            <button
              ...
              className="ueui-timeline-playhead-handle"
              ...
              type="button"
            >
              <span className="ueui-timeline-playhead-knob" />
            </button>
          </div>

          <div className="ueui-timeline-body" data-testid="semantic-timeline">
            <div className="ueui-timeline-scroll" onScroll={() => setContextMenu(null)}>
              <div className="ueui-timeline-lanes">
                ...
                {document.data.roles.length === 0 && signalLanes.length === 0 && !hasWorldInteractions ? (
                  <p className="ueui-timeline-empty">
...
function SignalRailLane({
  ...
  return (
    <div
      className="ueui-lane-signal"
      data-testid={`timeline-signal-lane-${laneId}`}
    >
      <div className="ueui-lane-signal-identity">
        <div
          className="ueui-timeline-identity-content ueui-timeline-identity-content-row"
          {...{ [IDENTITY_CONTENT_ATTR]: "" }}
        >
          {onFocus || onConfigure ? (
            <button
              aria-label={`${onFocus ? "Focus" : "Configure"} traffic light ${lane.referenceHeadId}`}
              className="ueui-lane-signal-focus"
              data-testid={`timeline-focus-signal-${laneId}`}
              type="button"
              onClick={onFocus ?? onConfigure}
            >
              <TrafficCone aria-hidden="true" size={12} />
              <span className="ueui-nowrap">Light {lane.referenceHeadId}</span>
            </button>
          ) : (
            <div className="ueui-lane-signal-label">
              <TrafficCone aria-hidden="true" size={12} />
              <span className="ueui-nowrap">Lights {lane.headIds.join(", ")}</span>
            </div>
          )}
          {onRemoveControl ? (
            <button
              aria-label={`Remove control from traffic light ${lane.referenceHeadId}`}
              className="ueui-lane-signal-remove"
      ...
      <div className="ueui-lane-track-signal" data-timeline-track="signal">
        {lane.bands.map((band) => {
          const colors = indicationSwatch(band.indication);
          ...
          return (
            <button
              ...
              className={selectable ? "ueui-signal-band ueui-signal-band-selectable" : "ueui-signal-band"}
              ...
              style={{
                ...
                ...(authored
                  ...
                  : { background: colors.ghost, borderStyle: "dashed", borderColor: "rgba(255,255,255,.2)" }),
              }}
              type="button"
            />
          );
...
function WorldRailLane({
  ...
  return (
    ...
    >
      <div
        className="ueui-lane-world-identity"
      ...
      >
        ...
        >
          ...
          <span className="ueui-nowrap ueui-lane-label">Scene / world</span>
        </div>
      </div>
...
function ActorRailLane({
  ...
  return (
    ...
    >
      <div
        className={selected ? "ueui-lane-actor-identity ueui-lane-actor-identity-selected" : "ueui-lane-actor-identity"}
      ...
      >
        ...
        >
          <button
            ...
            className="ueui-lane-actor-focus"
          ...
          >
            ...
            <span className="ueui-nowrap ueui-lane-label" title={actorLabel}>
              {actorLabel}
            </span>
          </button>
          {carlaCompatibility ? (
          ...
          {!readOnly ? (
            <button
              ...
              className="ueui-lane-actor-delete"
      ...
      {staticActor && interactions.length === 0 ? (
        <div
          className="ueui-lane-static-note"
...
function ReasoningTraceLane({
  ...
  return (
    <article className="ueui-lane-reasoning" data-testid="timeline-reasoning-trace-lane">
      <div className="ueui-lane-reasoning-identity">
        <div
          className="ueui-timeline-identity-content ueui-timeline-identity-content-row"
          {...{ [IDENTITY_CONTENT_ATTR]: "" }}
        >
          <BrainCircuit aria-hidden="true" size={12} />
          <span className="ueui-nowrap ueui-lane-reasoning-title">Reasoning</span>
        </div>
      </div>
      <div
        className="ueui-lane-track-reasoning"
        ...
        {segments.map((segment) => {
          ...
          return (
            <button
              ...
              className={selectedId === segment.id ? "ueui-reasoning-clip ueui-reasoning-clip-selected" : "ueui-reasoning-clip"}
              data-timeline-seek-ignore="true"
              ...
              type="button"
            >
              <span className="ueui-truncate">{label}</span>
            </button>
          );
        })}
      </div>
...
function ReasoningTraceEditor({
  ...
  return (
    <EditorDetailsPanel
      ...
      preview={(
        <div className="ueui-trace-preview">
          <BrainCircuit aria-hidden="true" size={32} />
          <strong>Reasoning trace</strong>
          <span>Observation and action</span>
        </div>
      )}
      testId="uniscenario-reasoning-trace-panel"
    >
      <div className="ueui-trace-grid">
        {(['startS', 'endS'] as const).map((field) => (
          <label className="ueui-trace-field" key={field}>{field === 'startS' ? 'Start' : 'End'}
            <input className="ueui-trace-input" disabled={readOnly} max={clipSeconds} min={field === 'startS' ? 0 : 0.1} onChange={(event) => setDraft({ ...draft, [field]: Number(event.target.value) })} step="0.1" type="number" value={draft[field]} />
          </label>
        ))}
      </div>
      <label className="ueui-trace-field">Observation
        <textarea className="ueui-trace-textarea" disabled={readOnly} onChange={(event) => setDraft({ ...draft, observation: event.target.value })} placeholder="What is happening around the camera vehicle?" value={draft.observation} />
      </label>
      <label className="ueui-trace-field">Action
        <textarea className="ueui-trace-textarea" disabled={readOnly} onChange={(event) => setDraft({ ...draft, action: event.target.value })} placeholder="What should the camera vehicle do next?" value={draft.action} />
      </label>
      {!readOnly ? <div className="ueui-trace-actions"><button className="ueui-trace-save" disabled={!Number.isFinite(draft.startS) || !Number.isFinite(draft.endS) || draft.startS < 0 || draft.endS <= draft.startS || draft.endS > clipSeconds} onClick={() => onSave(draft)} type="button">Save trace</button><button className="ueui-trace-delete" onClick={() => onDelete(draft.id)} type="button"><Trash2 aria-hidden="true" size={12} />Delete trace</button></div> : null}
    </EditorDetailsPanel>
  );
}
...
function InteractionBand({
  ...
  const clipVariantClass = routeNeedsSetup
    ? "ueui-interaction-clip-needs-setup"
    : cue?.conflict === "conflict"
      ? "ueui-interaction-clip-conflict"
      : cue?.conflict === "possible"
        ? "ueui-interaction-clip-possible"
        : resolved.armed || !editable
          ? "ueui-interaction-clip-armed"
          : "ueui-interaction-clip-solid";

  return (
    <div
      className="ueui-interaction-row"
    ...
    >
      <div className="contents" data-testid="interaction-track">
        <div className="ueui-interaction-hairline" />
        {deadlinePercent !== null ? (
          <span
            aria-label={`Trigger deadline by ${cue?.deadlineS}s`}
            className="ueui-interaction-deadline"
        ...
        <div
          className={cn(
            "ueui-interaction-clip",
            clipVariantClass,
            selected ? "ueui-interaction-clip-selected" : "",
          )}
        ...
        >
          {!timingLocked ? (
            <button
              ...
              className="ueui-interaction-resize ueui-interaction-resize-start"
          ...
          <button
            aria-controls={`uniscenario-interaction-${interaction.id}`}
            ...
            className="ueui-interaction-button"
          ...
          >
            {cue ? (
              <span
                className="ueui-interaction-cause"
                data-cause={cue.cause}
                data-testid={`timeline-cause-${interaction.id}`}
                title={timingHelp}
              >
                {cue.cause === "time" ? (
                  <Clock3 aria-hidden="true" size={6} />
                ) : (
                  <Zap aria-hidden="true" size={6} />
                )}
            ...
            {conflictMessage ? (
              <AlertTriangle
                ...
                size={10}
                className="ueui-interaction-conflict-icon"
          ...
          {!timingLocked && !endsWithScenario ? (
            <button
              ...
              className="ueui-interaction-resize ueui-interaction-resize-end"
...
function ContextActionMenu({
  ...
  return createPortal(
    ...
    >
      <header className="ueui-context-menu-header">
        <span className="ueui-context-menu-icon">
          <Plus aria-hidden="true" size={16} />
        </span>
        <div className="ueui-context-menu-heading">
          <p className="ueui-context-menu-kicker">
            Add at {state.timeS.toFixed(1)}s
          </p>
          <p className="ueui-context-menu-subtitle">{actorLabel}</p>
        </div>
        <button
          aria-label="Close action menu"
          className="ueui-context-menu-close"
          onClick={onClose}
          type="button"
        >
          ×
      ...
      <div className="ueui-context-menu-grid">
        {groups.map((group) => (
          <ActionMenuGroup key={group.label} label={group.label}>
            {group.actions.map((action) => (
              ...
              >
                <span className="ueui-context-action-inner">
                  {action.id === 'custom_route' ? <RouteIcon aria-hidden="true" size={14} /> : null}
        ...
        <div className="ueui-context-menu-span">
...
function ActionMenuGroup({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <section
      aria-label={label}
      className="ueui-context-group"
      data-testid={`timeline-context-group-${label.toLowerCase().replaceAll(" ", "-")}`}
      role="group"
    >
      <h3 className="ueui-context-group-title">
        {label}
      </h3>
      <div className="ueui-context-group-grid">{children}</div>
    </section>
  );
}
...
function TimelineActorCatalogIcon({ role }: { role: Role }) {
  ...
  const className = "ueui-timeline-actor-icon";
  ...
  if (catalogId && isPedestrianTimelineCatalogId(catalogId)) {
    return (
      <span aria-hidden="true" className="ueui-timeline-actor-icon ueui-timeline-actor-icon-small" data-testid={`timeline-actor-icon-${role.id}`}>
  ...
  if (role.actor.class === "pedestrian") {
    return <PersonStanding aria-hidden="true" size={12} className="ueui-timeline-actor-glyph" data-testid={`timeline-actor-icon-${role.id}`} />;
  }
  if (role.actor.class === "static_object") {
    return <Box aria-hidden="true" size={12} className="ueui-timeline-actor-glyph" data-testid={`timeline-actor-icon-${role.id}`} />;
  }
  return <CarFront aria-hidden="true" size={12} className="ueui-timeline-actor-glyph" data-testid={`timeline-actor-icon-${role.id}`} />;
}
