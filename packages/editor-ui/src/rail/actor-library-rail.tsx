...
import type { CarlaObjectDto } from "../carla-objects";
import {
...
} from "../catalog/panel-tiles";
import { PanelSearchResults } from "../catalog/panel-search-results";
import { CatalogTile } from "../catalog/catalog-tile";
import {
  Bike,
  Bird,
  Bot,
...
export type RailIconComponent = ComponentType<SVGProps<SVGSVGElement> & { size?: number | string; strokeWidth?: number }>;

/**
 * Product-supplied bodies for the tools whose panels are product concerns:
 * the asset gallery (uploads), the weather and traffic editors, and the
 * parked-car generator. The rail renders them inside the same drawer chrome.
 */
export interface ActorLibraryScenePanels {
  gallery?: ReactNode;
  weather?: ReactNode;
  traffic?: ReactNode;
  parked?: ReactNode;
...
export function ActorLibraryRail({
  ...
  carlaCompatibilityTable = null,
  carlaObjects = [],
  scenePanels = null,
  provideSceneSearchResults,
}: {
  ...
  document?: EditorDocument | null;
  /**
   * The immutable CARLA compatibility table. Loading it (an API fetch in the
   * product) stays with the surface that owns data fetching.
   */
  carlaCompatibilityTable?: CarlaCompatibilityTable | null;
  /** Measured CARLA objects merged into the bundled catalog sections. */
  carlaObjects?: readonly CarlaObjectDto[];
  /** Product panels for the gallery / weather / traffic / parked tools. */
  scenePanels?: ActorLibraryScenePanels | null;
  /**
   * Universal-search hits from the scene panels. Each panel knows how to apply
   * its own choice; the search surface only renders them.
   */
  provideSceneSearchResults?: (query: string, document: EditorDocument | null) => readonly SceneSearchResult[];
}) {
  ...
  const compatibilityByCatalogId = useMemo<Record<string, CarlaCompatibility> | null>(() => {
    if (!carlaCompatibilityTable) return null;
    return Object.fromEntries(
      catalog.map((entry) => [entry.id, carlaCompatibilityFor(entry.id, carlaCompatibilityTable)]),
    );
  }, [carlaCompatibilityTable, catalog]);
  ...
  const searchSceneResults = useMemo<readonly SceneSearchResult[]>(
    () => activeKind !== "search" || !query.trim() || !provideSceneSearchResults
      ? []
      : provideSceneSearchResults(query, editorDocument),
    [activeKind, editorDocument, provideSceneSearchResults, query],
  );
  ...
  return (
    ...
    >
      ...
      */}
      <div
        className="ueui-rail-row"
        data-testid="editor-tool-row"
        style={{ maxHeight: `calc(100% - ${RAIL_MARGIN * 2 + timelineInset}px)` }}
      >
      <nav
        style={{
          ...styles.rail,
      ...
      {open && activeTool ? (
        <section
          ref={catalogPanelRef as RefObject<HTMLElement>}
          aria-label={addActorTitle(activeTool)}
          className="ueui-add-panel-enter"
        ...
        >
          ...
          <div
            ...
            className="ueui-add-panel-resize"
          ...
          <div style={styles.controls}>
            ...
            <div style={styles.filterRow}>
              <div aria-label="Catalog filters" style={styles.filters}>
                {FILTERS.map((item) => (
                  <button
                    className="ueui-chip"
                    key={item.id}
            ...
            {categories.length > 1 ? (
              <div aria-label="Categories" role="group" style={styles.categories}>
                <button
                  ...
                  className="ueui-chip ueui-catalog-chip-enter"
          ...
          <div style={styles.panelBody}>
            {activeKind === "search" ? (
              ...
              scenePanels?.weather ?? null
            ) : activeTool === "traffic" ? (
              scenePanels?.traffic ?? null
            ) : activeTool === "parked" && scenePanels?.parked ? (
              scenePanels.parked
            ) : activeTool === "gallery" && controller ? (
              scenePanels?.gallery ?? null
              ...
              <div style={styles.empty}>
                ...
                <span>
                  {carlaCompatibleOnly
                    ? `CARLA ${carlaCompatibilityTable?.carlaVersion ?? "0.10.0"} has no blueprint for this category.`
...
const styles: Record<string, CSSProperties> = {
  ...
  // `.ueui-catalog-tile`: hover cannot be expressed inline.
