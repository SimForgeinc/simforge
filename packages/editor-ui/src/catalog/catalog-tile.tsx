...
import type { CatalogEntry, CatalogId, PropClass } from "@uniscenarios/prop-catalog";
...
import { DynamicActorCatalogIcon, type DynamicActorCatalogId } from "./dynamic-actor-catalog-icon";
import { ObjectCatalogIcon } from "./object-catalog-icon";
import { PedestrianCatalogIcon, type PedestrianCatalogId } from "./pedestrian-catalog-icon";
import { VehicleCatalogIcon, type VehicleCatalogId } from "./vehicle-catalog-icon";
...
export function CatalogTile({
  ...
  return (
    ...
    >
      ...
      >
        <span className="ueui-catalog-tile-icon" style={{ ...styles.tileIcon, color: classColor(entry.class) }}>{icon}</span>
        <strong className="ueui-catalog-tile-label" style={styles.tileLabel}>{entry.label}</strong>
        <span className="ueui-catalog-tile-meta" style={styles.tileMeta}>{entry.dims.l.toFixed(1)} × {entry.dims.w.toFixed(1)} m</span>
      </div>
