...
# Usage: tools/golden-harness/ci-local.sh [record]   # 'record' re-records goldens first
set -euo pipefail
...
if [ "${1:-verify}" = "record" ]; then
  for scene in tools/golden-harness/scenes/*.json; do
    node tools/golden-harness/golden.mjs record "$(basename "$scene" .json)"
  done
fi
