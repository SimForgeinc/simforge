 ...
 *
 * Usage:
 *   node tools/realism-ablation/run-ablation.mjs \
 ...
 * runner then tries `pnpm --filter @uniscenarios/bridge-fidelity exec node dist/score-cli.js %FRAMES%`.
 */
...
async function main() {
  ...
  const spikeOut = path.resolve(args.get('spike-out') ?? '/home/path/UniScenarios/scripts/renderer-spike/out');
  ...
  //   uv run --project tools/bridge-fidelity bf-score --translated <frames> \
  ...
  const scorecardCmd = args.get('scorecard-cmd') ?? [
    'uv', 'run', '--project', path.join(repoRoot, 'tools/bridge-fidelity'), 'bf-score',
    '--translated', '%FRAMES%',
  ...
  const verdictJson = {
    ...
    notes: [
      ...
      ...(comparable ? [] : ['Stage B completes with: node tools/realism-ablation/run-ablation.mjs --stage-a <dir> --stage-b <bevyFramesDir> --out <verdict.json>']),
    ],
  };
  ...
  if (record) {
    const evidenceDir = path.join(repoRoot, 'tools/realism-ablation/evidence');
